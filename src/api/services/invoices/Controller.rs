...
impl Controller {
    ...
    async fn getInvoiceHandler(service: web::Data<InvoiceServiceManager>) -> impl Responder {
        web::Json(Invoice{ customerId: "".to_string(), code: format!("{} {}", "Call me baby INVOICE : ", service.getInvoice().await.to_string()) })
    }

    async fn getInvoicesHandler() -> impl Responder {
        web::Json(Invoice{ customerId: "".to_string(), code: "Call me baby INVOICE : ".to_owned() })
    }
